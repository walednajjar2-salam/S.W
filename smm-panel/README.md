...
Worker (كل ~2 ثانية)
       ↓
الحسابات المولَّدة المربوطة تضغط متابعة/لايك على رابط الطلب
       ↓
delivered += عدد الضغطات (أو 8% محاكاة إن لم يتبقَّ حسابات)
       ↓
delivered >= quantity → status: completed
```

**لا يوجد** إرسال متابعين حقيقي لأي منصة. الضغط يتم داخل اللوحة على الرابط المطلوب في الطلب.
...
1. **توليد حسابات:** حتى **10000 حساب في حزمة واحدة** (زر «حزمة 10000»). كلمة المرور موحدة للحزمة.
2. **ربط تلقائي:** عند التوليد يُربط كل حساب بإنستجرام وتيك توك داخل اللوحة (`@user1` …) ويُستخدم الرابط تلقائياً في الطلبات، ثم تضغط هذه الحسابات متابعة/لايك على رابط كل طلب.
...
- `GET /api/orders/{id}/engagement` — من ضغط متابعة/لايك على رابط الطلب
